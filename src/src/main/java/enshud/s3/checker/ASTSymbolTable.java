...
public class ASTSymbolTable
{
	...
	public String getScope(final ArrayDeque<String> scope, final String variableName)
	{
		...
		{
			String global=scope.getFirst()+"."+variableName;
			sn=scope.stream().reduce("", (joined, s)->joined+s+".")+variableName;
			// global variable
			table.stream().filter(f->f.getName().equals(scope.getLast())).findFirst().ifPresent(e->
			{
				e.addUsedGlobalVariable(global, getVariableType(variableName));
				e.addGlobalAndLocalVariableCorrespondence(global, sn);
